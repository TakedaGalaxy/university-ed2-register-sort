  ...
  foreach ($bSize in $bSizes) {
    foreach ($sSize in $sSizes) {
      $executionTime = Measure-ExecutionTime { 
        & ".\out\main.exe" -s ".\out\data-$fileSize.bin" $bSize ($bSize / $sSize) ".\out\data-sorted-$fileSize-B$($bSize)-S$($sSize).bin"
      }

      Write-Output "F$fileSize B$bSize S$sSize $executionTime ms"
    }
  }
}
